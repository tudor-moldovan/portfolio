...
// Minimal shape check. Reject garbage; otherwise trust the edit page.
function normalize(arr) {
  ...
  for (const raw of arr) {
    ...
    if (typeof raw.units !== 'number' || raw.units <= 0) throw new Error(`${raw.symbol}: units must be > 0`);
    if (typeof raw.avgCost !== 'number' || raw.avgCost <= 0) throw new Error(`${raw.symbol}: avgCost must be > 0`);
    out.push({
      symbol: raw.symbol,
...
export default async function handler(req) {
  ...
  if (req.method === 'PUT') {
    if (!checkAuth(req)) return new Response(JSON.stringify({ error: 'unauthorized' }), { status: 401, headers: { 'Content-Type': 'application/json', ...CORS } });
    if (!kv) return new Response(JSON.stringify({ error: 'KV not configured' }), { status: 503, headers: { 'Content-Type': 'application/json', ...CORS } });
    let body;
    ...
    let positions;
    try { positions = normalize(raw); } catch (e) { return new Response(JSON.stringify({ error: e.message }), { status: 400, headers: { 'Content-Type': 'application/json', ...CORS } }); }
    await kv.set('positions:current', JSON.stringify(positions));
    return new Response(JSON.stringify({ ok: true, count: positions.length }), { headers: { 'Content-Type': 'application/json', ...CORS } });
  }
